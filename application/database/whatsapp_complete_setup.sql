-- WhatsApp Complete Setup for Tenant Database
-- Run this SQL in your tenant database to enable full WhatsApp functionality
-- Based on growcrm_landlord.sql structure + additional tables needed for integration
...
-- Insert default ticket types (UPDATE tenant_id to match your actual tenant ID)
INSERT INTO `whatsapp_ticket_types` (`tenant_id`, `name`, `description`, `color`, `is_active`, `sort_order`, `created_at`, `updated_at`) VALUES
(1, 'Soporte', 'Soporte técnico y asistencia', '#007bff', 1, 1, NOW(), NOW()),
(1, 'Ventas', 'Consultas y preguntas de ventas', '#28a745', 1, 2, NOW(), NOW()),
(1, 'Facturación', 'Problemas de facturación y pago', '#ffc107', 1, 3, NOW(), NOW()),
(1, 'General', 'Consultas e información general', '#6c757d', 1, 4, NOW(), NOW());

-- Insert default quick templates (UPDATE tenant_id to match your actual tenant ID)
INSERT INTO `whatsapp_quick_templates` (`tenant_id`, `name`, `content`, `category`, `is_active`, `sort_order`, `created_at`, `updated_at`) VALUES
(1, 'Saludo', '¡Hola! Gracias por contactarnos. ¿Cómo puedo ayudarte hoy?', 'greeting', 1, 1, NOW(), NOW()),
(1, 'Gracias', 'Gracias por tu mensaje. ¡Agradecemos tu negocio!', 'closing', 1, 2, NOW(), NOW()),
(1, 'Seguimiento', 'Haré un seguimiento de esto y me pondré en contacto contigo en breve.', 'follow-up', 1, 3, NOW(), NOW()),
(1, 'Solicitud de Soporte', 'Entiendo que necesitas soporte técnico. Permíteme ayudarte con eso.', 'support', 1, 4, NOW(), NOW());

-- Insert default tags (UPDATE tenant_id to match your actual tenant ID)
INSERT INTO `whatsapp_tags` (`tenant_id`, `name`, `color`, `is_active`, `created_at`, `updated_at`) VALUES
(1, 'VIP', '#dc3545', 1, NOW(), NOW()),
(1, 'Nuevo Cliente', '#28a745', 1, NOW(), NOW()),
(1, 'Urgente', '#fd7e14', 1, NOW(), NOW()),
(1, 'Seguimiento Requerido', '#6f42c1', 1, NOW(), NOW());

-- Insert a sample WhatsApp connection (UPDATE tenant_id to match your actual tenant ID)
...
(1, 'Main WhatsApp Business', 'baileys', 'disconnected', '+1234567890', 1, NOW(), NOW());
...
-- =====================================================
-- IMPORTANT: Update tenant_id values above to match your actual tenant ID
-- =====================================================
-- 
-- To find your tenant ID, run this query in your tenant database:
-- SELECT id FROM tenants LIMIT 1;
-- 
-- Then update all the INSERT statements above to use your actual tenant ID
-- instead of the placeholder value "1"
--
-- =====================================================
