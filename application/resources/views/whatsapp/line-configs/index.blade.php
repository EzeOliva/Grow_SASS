...
<div class="container-fluid">
	<div class="row page-titles mb-4">
		...
		<div class="col-md-6 text-end">
			<a href="{{ route('whatsapp.line-configs.create') }}" class="btn btn-primary btn-lg shadow-sm">
			...
			<div class="row align-items-center">
				...
				<div class="col-md-6 text-end">
					<span class="badge bg-light text-dark fs-6 px-3 py-2">
						<i class="fas fa-info-circle me-1"></i>{{ isset($lineConfigs) ? $lineConfigs->count() : 0 }} Lines
					</span>
				</div>
			</div>
		</div>
		<div class="card-body p-0">
			@if(isset($lineConfigs) && $lineConfigs->count())
								...
								<tr class="border-bottom">
									<td class="py-3 px-4">
										<div class="d-flex align-items-center">
											...
											<div>
												<h6 class="text-dark fw-semibold mb-1">{{ $lineConfig->line_name }}</h6>
												<small class="text-muted">
													<i class="fas fa-clock me-1"></i>
													@if($lineConfig->inactivity_timeout_minutes)
														{{ $lineConfig->inactivity_timeout_minutes }} min timeout
													@else
														No timeout set
													@endif
												</small>
									...
									<td class="py-3 px-4">
										@if($lineConfig->connection)
											<span class="badge bg-info bg-opacity-10 text-info fs-6 px-3 py-2">
												<i class="fas fa-plug me-1"></i>{{ $lineConfig->connection->connection_name }}
											</span>
										@else
									...
									<td class="py-3 px-4">
										<div class="d-flex flex-column">
											<span class="badge bg-warning bg-opacity-10 text-warning fs-6 px-3 py-2 mb-1">
												<i class="fas fa-cogs me-1"></i>{{ ucfirst(str_replace('_', ' ', $lineConfig->assignment_mode)) }}
											</span>
											@if($lineConfig->auto_assign_enabled)
									...
									<td class="py-3 px-4">
										<span class="badge {{ $lineConfig->is_active ? 'bg-success' : 'bg-secondary' }} fs-6 px-3 py-2">
											<i class="fas {{ $lineConfig->is_active ? 'fa-check-circle' : 'fa-times-circle' }} me-1"></i>
											{{ $lineConfig->is_active ? 'Active' : 'Inactive' }}
										</span>
									</td>
									<td class="py-3 px-4 text-center">
										<div class="btn-group" role="group">
											<a href="{{ route('whatsapp.line-configs.show', $lineConfig) }}" class="btn btn-outline-primary btn-sm me-2">
												<i class="fas fa-eye me-1"></i>View
											</a>
											<a href="{{ route('whatsapp.line-configs.edit', $lineConfig) }}" class="btn btn-outline-warning btn-sm">
				...
				@if(method_exists($lineConfigs, 'links'))
			...
			@else
				<div class="text-center py-5">
					<div class="empty-state">
						...
						<a href="{{ route('whatsapp.line-configs.create') }}" class="btn btn-primary">
