...
<div class="container-fluid">
	<div class="row page-titles mb-4">
		...
		<div class="col-md-6 text-end">
			<a href="{{ route('whatsapp.tickets.create') }}" class="btn btn-primary btn-lg shadow-sm me-2">
				<i class="fas fa-plus-circle me-2"></i>Create Ticket
			</a>
			<a href="{{ route('whatsapp.dashboard') }}" class="btn btn-outline-info btn-lg shadow-sm">
	...
	<div class="row mb-4">
		<div class="col-12">
			<div class="card border-warning">
				...
				<div class="card-body">
					<p><strong>Tickets Count:</strong> {{ isset($tickets) ? $tickets->count() : 'Not set' }}</p>
					<p><strong>Tickets Total:</strong> {{ isset($tickets) && method_exists($tickets, 'total') ? $tickets->total() : 'Not set' }}</p>
					<p><strong>KPIs:</strong> {{ isset($kpis) ? json_encode($kpis) : 'Not set' }}</p>
					<p><strong>Agents Count:</strong> {{ isset($agents) ? $agents->count() : 'Not set' }}</p>
					@if(isset($error))
						<p><strong>Error:</strong> <span class="text-danger">{{ $error }}</span></p>
					@endif
					@if(isset($debug_info))
						<hr>
						<h6>Additional Debug Info:</h6>
						<p><strong>Database:</strong> {{ $debug_info['database'] ?? 'Not set' }}</p>
						<p><strong>Table Exists:</strong> {{ $debug_info['table_exists'] ? 'Yes' : 'No' }}</p>
						<p><strong>Ticket Count:</strong> {{ $debug_info['ticket_count'] ?? 'Not set' }}</p>
						<p><strong>Raw Tickets:</strong> <pre>{{ json_encode($debug_info['raw_tickets'] ?? [], JSON_PRETTY_PRINT) }}</pre></p>
					@endif
				</div>
			</div>
		</div>
			...
			<div class="card border-0 shadow-sm bg-gradient-primary text-white">
				<div class="card-body p-4">
					<div class="d-flex align-items-center">
						...
						<div>
							<h4 class="mb-1 fw-bold">{{ $kpis['total_tickets'] ?? 0 }}</h4>
			...
			<div class="card border-0 shadow-sm bg-gradient-warning text-white">
				<div class="card-body p-4">
					<div class="d-flex align-items-center">
						...
						<div>
							<h4 class="mb-1 fw-bold">{{ $kpis['open_tickets'] ?? 0 }}</h4>
			...
			<div class="card border-0 shadow-sm bg-gradient-info text-white">
				<div class="card-body p-4">
					<div class="d-flex align-items-center">
						...
						<div>
							<h4 class="mb-1 fw-bold">{{ $kpis['in_progress_tickets'] ?? 0 }}</h4>
			...
			<div class="card border-0 shadow-sm bg-gradient-success text-white">
				<div class="card-body p-4">
					<div class="d-flex align-items-center">
						...
						<div>
							<h4 class="mb-1 fw-bold">{{ $kpis['closed_tickets'] ?? 0 }}</h4>
			...
			<div class="row align-items-center">
				...
				<div class="col-md-6 text-end">
					<span class="badge bg-light text-dark fs-6 px-3 py-2">
						<i class="fas fa-info-circle me-1"></i>{{ isset($tickets) ? $tickets->count() : 0 }} Tickets
					</span>
				</div>
			</div>
		</div>
						...
						<tbody>
							@foreach($tickets as $ticket)
								<tr class="border-bottom">
										...
										<div class="d-flex align-items-center">
											...
											<div>
												<h6 class="text-dark fw-semibold mb-1">
													#{{ $ticket->id }} - {{ strlen($ticket->subject) > 50 ? substr($ticket->subject, 0, 50) . '...' : $ticket->subject }}
												</h6>
												@if($ticket->ticketType)
													<small class="text-muted">
														<i class="fas fa-tag me-1"></i>{{ $ticket->ticketType->name }}
													</small>
												@endif
											</div>
										</div>
									...
									<td class="py-3 px-4">
										@if($ticket->contact)
											<div class="d-flex align-items-center">
												...
												<div>
													<div class="fw-semibold">{{ $ticket->contact_name ?? 'Unknown' }}</div>
													<small class="text-muted">{{ $ticket->contact_phone ?? 'No phone' }}</small>
												</div>
											</div>
										@else
									...
									<td class="py-3 px-4">
										@if($ticket->agent)
											<span class="badge bg-info bg-opacity-10 text-info fs-6 px-3 py-2">
												<i class="fas fa-user-tie me-1"></i>{{ $ticket->agent->first_name }} {{ $ticket->agent->last_name }}
											</span>
										@else
									...
									<td class="py-3 px-4">
										<span class="badge bg-{{ $ticket->status === 'open' ? 'warning' : ($ticket->status === 'in_progress' ? 'info' : 'success') }} fs-6 px-3 py-2">
											<i class="fas {{ $ticket->status === 'open' ? 'fa-clock' : ($ticket->status === 'in_progress' ? 'fa-spinner' : 'fa-check-circle') }} me-1"></i>
									...
									<td class="py-3 px-4">
										<small class="text-muted">
											...
											{{ $ticket->created_at ? $ticket->created_at->diffForHumans() : 'Unknown' }}
										</small>
									</td>
									<td class="py-3 px-4 text-center">
										<div class="btn-group" role="group">
											<a href="{{ route('whatsapp.tickets.show', $ticket) }}" class="btn btn-outline-primary btn-sm me-2">
												<i class="fas fa-eye me-1"></i>View
											</a>
											<a href="{{ route('whatsapp.tickets.edit', $ticket) }}" class="btn btn-outline-warning btn-sm">
				...
				<div class="text-center py-5">
					<div class="empty-state">
						...
						<p class="text-muted fs-6 mb-4">
							@if(isset($error))
								<strong class="text-danger">Error:</strong> {{ $error }}
							@else
						...
						<div class="d-flex gap-2 justify-content-center">
							<a href="{{ route('whatsapp.tickets.create') }}" class="btn btn-primary">
								<i class="fas fa-plus me-2"></i>Create First Ticket
							</a>
							@if(!isset($error))
								<a href="{{ route('whatsapp.dashboard') }}" class="btn btn-outline-secondary">
