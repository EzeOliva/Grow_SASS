    ...
    <div class="whatsapp-card mb-4">
        <div class="card-header">
            ...
            <div class="d-flex align-items-center gap-3">
                ...
                <span class="channel-badge channel-{{ $ticket->channel }}">
                    <i class="fab fa-{{ $ticket->channel == 'whatsapp' ? 'whatsapp' : 'envelope' }} me-1"></i>
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-6">
                    ...
                    <p><strong>Email:</strong> {{ $ticket->contact_email ?: 'Not provided' }}</p>
                ...
                <div class="col-md-6">
                    ...
                    <p><strong>Opened:</strong> {{ $ticket->opened_at ? $ticket->opened_at->format('M d, Y H:i') : 'Not set' }}</p>
                    <p><strong>Agent:</strong> 
                        @if($ticket->agent && $ticket->agent->first_name)
                    ...
                    @if($ticket->first_response_at)
                        <p><strong>First Response:</strong> {{ $ticket->first_response_at->format('M d, Y H:i') }}</p>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="whatsapp-card">
        ...
        <div class="card-body">
            @foreach(($messages ?? []) as $message)
                <div class="message {{ $message->isFromClient() ? 'message-client' : 'message-agent' }} mb-3">
                        ...
                        <div class="message-header mb-2">
                            <strong>
                                @if($message->isFromClient())
                                    {{ $message->sender_name }}
                                @else
                                    {{ $message->sender ? $message->sender->first_name . ' ' . $message->sender->last_name : $message->sender_name }} (Agent)
                                @endif
                            </strong>
                            <small class="text-muted ms-2">{{ $message->created_at->format('M d, Y H:i') }}</small>
                        </div>
    ...
    @if($ticket->status !== 'closed')
        <div class="whatsapp-card mt-4">
            <div class="card-body">
                ...
                <form method="POST" action="{{ route('whatsapp.tickets.send-message', $ticket) }}" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
                        ...
                        <div class="composer-toolbar mb-2">
                            ...
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu templates-menu" aria-labelledby="templatesDropdown">
                                    @forelse(($canned_responses ?? []) as $canned)
                                        <li>
                                            <a class="dropdown-item js-insert-template" href="#" data-template="{!! str_replace(["\n", "\r"], '', addslashes($canned->canned_message)) !!}">
                                                <div class="fw-bold">{{ $canned->canned_title }}</div>
                                                <small class="text-muted">{{ $canned->category_name }}</small>
                                            </a>
                                        </li>
                                    @empty
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="channel" id="replyChannel" onchange="toggleReplyApiSelection()" required style="height: 35px; font-size: 15px;">
                            <option value="whatsapp" {{ $ticket->channel == 'whatsapp' ? 'selected' : '' }}>WhatsApp</option>
                            <option value="email" {{ $ticket->channel == 'email' ? 'selected' : '' }}>Email</option>
                        </select>
                    </div>
                    ...
                    <button type="submit" class="btn btn-whatsapp">
                        <i class="fab fa-{{ $ticket->channel == 'whatsapp' ? 'whatsapp' : 'envelope' }} me-2"></i>
                        Send Message
                    </button>
                </form>
            </div>
...
<script>
    ...
    async function sendTestWhatsAppMessage() {
        ...
        const message = document.getElementById('replyMessage').value;
        const phoneNumber = '{{ $ticket->contact_phone }}';
    ...
    async function sendTestEmailMessage() {
        ...
        try {
            const data = {
                ...
                email: email,
                subject: 'Test Reply - Ticket #{{ $ticket->id }}',
                message: message
            };
