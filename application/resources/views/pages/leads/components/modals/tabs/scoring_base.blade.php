<div class="card-header bg-light text-dark border-bottom">
    <h6 style="font-weight:600;"><i class="fa-solid fa-ranking-star"></i> Puntuación y Sugerencias</h6>
</div>
<div class="card-body">
    ...
    <div class="mb-3">
        ...
        <ul class="list-unstyled mb-2" style="font-size:15px;">
            ...
            <li><strong>Categoría:</strong> {{ $lead->leadcategory->leadcategory_title ?? 'N/A' }}</li>
            <li><strong>Último Contacto:</strong> {{ $lead->lead_last_contacted ? \Carbon\Carbon::parse($lead->lead_last_contacted)->format('M d, Y') : 'Nunca' }}</li>
            <li><strong>Usuarios Asignados:</strong> {{ $lead->assigned->count() ?? 0 }}</li>
            <li><strong>Comentarios:</strong> {{ $lead->comments->count() ?? 0 }}</li>
            <li><strong>Propuestas:</strong> {{ $lead->proposals->count() ?? 0 }}</li>
            <li><strong>Contratos:</strong> {{ $lead->contracts->count() ?? 0 }}</li>
        </ul>
    </div>
